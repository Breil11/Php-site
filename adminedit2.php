...
if(isset($_SESSION['id'])) {
   $requser = $usr->prepare("SELECT * FROM utilisateur WHERE id = ?");
   $requser->execute(array($_SESSION['id']));
   $user = $requser->fetch();
   if(isset($_POST['newnom']) AND !empty($_POST['newnom']) AND $_POST['newpnom'] != $user['nom']) {
      $newnom = htmlspecialchars($_POST['newnom']);
      $insertnom = $usr->prepare("UPDATE utilisateur SET nom = ? WHERE id = ?");
      $insertnom->execute(array($newnom, $_SESSION['id']));
      header('Location: admin.php?id='.$_SESSION['id']);
   }
   if(isset($_POST['newmail']) AND !empty($_POST['newmail']) AND $_POST['newmail'] != $user['mail']) {
      $newmail = htmlspecialchars($_POST['newmail']);
      $insertmail = $usr->prepare("UPDATE utilisateur SET mail = ? WHERE id = ?");
   ...
   if(isset($_POST['newmdp1']) AND !empty($_POST['newmdp1']) AND isset($_POST['newmdp2']) AND !empty($_POST['newmdp2'])) {
      ...
      $mdp2 = ($_POST['newmdp2']);
      if($mdp1 == $mdp2) {
         $insertmdp = $usr->prepare("UPDATE utilisateur SET mdp = ? WHERE id = ?");
